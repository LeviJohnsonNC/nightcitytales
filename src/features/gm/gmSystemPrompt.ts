...
- Whenever a check is aimed AT A PERSON but nobody is resisting — watching them for a tell, reading the room, listening to how they answer, sizing up a stranger across a bar — keep it a skill_check with a DV AND name them: set "npcKey" and "npcName" the same way an opposed check does, using the key printed in NPCS PRESENT when they are there. The engine decides what paying that much attention to somebody tells the character, and it cannot do that for a check that names nobody. Leave both out for a check against the world.
...
- "proposedActions": the mechanical actions the engine should resolve from the player's stated intent. Propose; do not resolve. Every item is an object whose discriminator field is named EXACTLY "kind". Use these shapes verbatim — a different field name means the engine never sees the action and the player never gets to roll:
  ...
  - {"kind": "use_item", "item": "<the thing they are using, as the kit list names it>", "quantity": <integer>, "intent": "<what they are doing with it>"} — propose this whenever they actually use something out of the kit. The engine decides whether using it USES IT UP: a flare, an ampoule, a grenade off the ammunition rows is spent; a lockpick set, a scanner, a rope is not. Never narrate them running out, and never tell them how many are left — the engine counts and the sheet says.
