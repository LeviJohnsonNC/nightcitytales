...
export const LIFE_PROMPT_VERSION = "2.13.0";
...
- Return 3-4 entries in "actions", drawn from what is already in the scene. Each states what the character would know before committing: roughly how long it takes, and a cost in eurobucks when the engine has told you the price. Never reveal hidden information, never promise an outcome, and never put a number on either one yourself.
- ${ROLE_MOVE_RULE}
...
${unknownFactsSection({
  examples:
    "is the clinic still open at this hour, did the neighbour hear the shot, is the fixer already in the bar",
  alsoKnown: "the character's own money, kit, wounds, plans or the numbers on the dials",
})}

${cityNoticedSection({
  quietTurn: "A quiet evening reports nothing, which is the normal answer.",
})}
...
${walkOnFacesSection({ excludeHostiles: false })}
...
- "proposedActions": what the engine should resolve, using EXACTLY these shapes:
  ...
  - {"kind":"travel","destination":"<EXACT name from the travel list in WHERE YOU ARE>","direction":"north|northeast|east|southeast|south|southwest|west|northwest","extent":"near|far","mode":"foot|cab","blocks":<integer>,"minutes":<integer>} — propose this whenever the player says they are heading somewhere.
    Set "blocks" when they put a DISTANCE on it — "about seven blocks", "a couple of streets down", "a few blocks that way". The engine walks or drives them that far along the heading and stops where they asked to stop, instead of carrying them to the next district. Leave it out when they only said which way.
    ...
    A heading can also simply run out. Walking west from the western edge of the Island ends at the water, not in another district, and the engine will tell you so. That is a real outcome, not a failure — the character really has moved, to the waterfront, and the clock really has run.
    A character stands at a SPOT, not in a district generally. Crossing Little Europe to its western shore is most of a kilometre and a quarter of an hour on foot, and the engine tracks it. So do not treat "still in the same district" as "nothing happened".
