...
nothing. Its migration is `20260904030000_campaign_places.sql` and it must be
applied before Life will load.
...
- The `goodwill` and `gang_pressure` dials in `place-state.json` move but no
  threshold reads them, so they currently accumulate without reaching the
  player. A dial nothing depends on is the failure `PRODUCT.md` warns about;
  either give them a threshold or take them out.
