...
CREATE POLICY "Users read their own portraits"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'portraits' AND (storage.foldername(name))[1] = auth.uid()::text);

CREATE POLICY "Users upload their own portraits"
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'portraits' AND (storage.foldername(name))[1] = auth.uid()::text);

CREATE POLICY "Users update their own portraits"
ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'portraits' AND (storage.foldername(name))[1] = auth.uid()::text)
WITH CHECK (bucket_id = 'portraits' AND (storage.foldername(name))[1] = auth.uid()::text);

CREATE POLICY "Users delete their own portraits"
...
BEGIN
  ...
  FOR v_entry IN SELECT * FROM jsonb_array_elements(coalesce(payload->'skills', '[]'::jsonb)) LOOP
    ...
    SELECT * FROM jsonb_populate_record(
      ...
      v_entry || jsonb_build_object('character_id', v_char)
    );
  END LOOP;
  ...
  FOR v_entry IN SELECT * FROM jsonb_array_elements(coalesce(payload->'gear', '[]'::jsonb)) LOOP
    ...
    SELECT * FROM jsonb_populate_record(
      ...
      (v_entry - 'key') || jsonb_build_object('character_id', v_char)
    )
  ...
  FOR v_entry IN SELECT * FROM jsonb_array_elements(coalesce(payload->'cyberware', '[]'::jsonb)) LOOP
    ...
    SELECT * FROM jsonb_populate_record(
      ...
      (v_entry - 'key' - 'foundation_key') || jsonb_build_object('character_id', v_char)
    )
    RETURNING id INTO v_new;
    IF v_entry ? 'key' THEN
      v_ids := v_ids || jsonb_build_object(v_entry->>'key', v_new);
