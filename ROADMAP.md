...
| 10  | Incomplete  | Final campaign status is not written by the play loop on anything but death (`status: "lost"`); a completed job leaves the campaign `active`, which may now be correct — a campaign is a life, not a job — and wants a decision rather than a patch. **Objectives are done**: an exit closes them (`completes`/`fails`), `validateMission` rejects an objective nothing can close, and settlement's `done/total` count and its `messy` flag are accurate for the first time. |
